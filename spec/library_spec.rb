...
describe Library do
  ...
  it "add new books and assigns it an id" do
    sample_book = Book.new("Nausea", "Jean-Paul Sartre")
    lib = Library.new
    lib.register_new_book(sample_book)
    expect(lib.books.count).to eq(1)

    created_book = lib.books.first
    expect(sample_book.title).to eq "Nausea"
    expect(sample_book.author).to eq "Jean-Paul Sartre"
    expect(sample_book.id).to_not be_nil
  end
  ...
  it "allows a Borrower to check out a book by its id" do
    ...
    lib.check_out_book(book_id, sam)
  ...
  it "knows who borrowed a book" do
    ...
    lib.check_out_book(book_id, bro)
  ...
  it "allows a Borrower to check a book back in" do
    book1 = Book.new("Finnegans Wake", "James Joyce")
    lib = Library.new
    lib.register_new_book(book1)
    book_id = lib.books.first.id
  ...
  it "does not allow a Borrower to check out more than two Books at any given time" do
    book_one = Book.new("Eloquent JavaScript", "Marijn Haverbeke")
    book_two = Book.new("Essential JavaScript Design Patterns", "Addy Osmani")
    book_three = Book.new("JavaScript: The Good Parts", "Douglas Crockford")
    lib = Library.new
    lib.register_new_book(book_one)
    lib.register_new_book(book_two)
    lib.register_new_book(book_three)
